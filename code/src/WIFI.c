...
void __attribute__((interrupt, no_auto_psv)) _U1RXInterrupt() 
{
    ...
    { 
      ...
      if(strstr("R", (const char*)buffer)!=NULL)
            {
                 LATBbits.LATB6 = 1;
                __delay_ms(500);
                LATBbits.LATB6 = 0;
    ...
    if(strstr((const char*)buffer, "<S>") && strstr((const char*)buffer, "<E>")) 
    {
            messageReady = 1;
            if(strstr("R", (const char*)buffer)!=NULL)
            {
...
void parseData() 
{
    char* start = strstr((char*)buffer, "<S>");
    char* end = strstr((char*)buffer, "<E>");
    ...
    if (start != NULL && end != NULL && end > start) 
    {
        ...
        while (token != NULL && i < COLOR_NUM) 
        {
            if (strcmp(token, "S") == 0) /* handle start sequence */
            {
                flag = 0;
            } 
            else if (strcmp(token, "R") == 0) /* handle stop/reset sequence */
            {
                flag = 1;
                stopAll();
            } 
